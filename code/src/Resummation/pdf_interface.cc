...
// LHA PDF inizialisation

  resuminfo.pdf_res.pdf_LHA = LHAPDF::mkPDF(resuminfo.pdf_setname, resuminfo.lha_imem);
  resuminfo.pdf_res.LHA_set = 1;
...
double PDF_res_interface::xfxQ(int pid, double x, double Q) const{
  ...
  double res;
  if(LHA_set == 1){
    // std::cout << "pid = " << pid << " x = " << x << " Q = " << Q << std::endl;
    res = pdf_LHA->xfxQ(pid, x, Q);
  }
  else if(MSTW_set == 1){
    int ii = pid;
    if(pid == 21) ii = 0;
    res = pdf_MSTW->parton(ii, x, Q);
  }
  else res = 0.;
