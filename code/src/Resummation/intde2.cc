...
void intdeoini(int lenaw, double tiny, double eps, double *aw)
{
    ...
    pi4 = std::atan(1.0);
    tinyln = -std::log(tiny);
    epsln = 1 - std::log(efs * eps);
    frq4 = 1 / (2 * pi4);
    per2 = 4 * pi4;
    pq = pqoff / epsln;
    pp = ppoff - std::log(pq * pq * frq4);
    ehp = std::exp(2 * pq);
    ehm = 1 / ehp;
    aw[3] = lmax;
    aw[4] = eps;
    aw[5] = std::sqrt(efs * eps);
    noff0 = 6;
    ...
    *err = std::abs(*i);
    ...
    do {
        ...
        do {
            if (k == noff) {
                ...
                do {
                    ...
                    xa = per * aw[j];
                    fm = (*f)(a + xa, userdata);
                    fp = (*f)(a + xa + perw * tk, userdata);
                    ir += (fm + fp) * aw[j + 1];
                    fm *= aw[j + 2];
                    fp *= w02 - aw[j + 2];
                    *i += fm + fp;
                    *err += std::abs(fm) + std::abs(fp);
                    tk += 1;
                } while (aw[j] > eps && j < k);
                errh = *err * aw[5];
                *err *= eps;
            ...
            while (std::abs(fm) > *err && j < k) {
                j += 3;
                fm = (*f)(a + per * aw[j], userdata);
                ir += fm * aw[j + 1];
                fm *= aw[j + 2];
                *i += fm;
            }
            fm = (*f)(a + perw * tk, userdata);

            s2 = w02 * fm;
            *i += s2;
            if (std::abs(fp) > *err || std::abs(s2) > *err) {
                l = 0;
                for (;;) {
                    l++;
                    s0 = 0;
        ...
        if (m == 1) {
            ...
            errd = h * (std::abs(*i - 2 * iback) + std::abs(ir - 2 * irback));
        }
        h *= 0.5;
        m *= 2;
