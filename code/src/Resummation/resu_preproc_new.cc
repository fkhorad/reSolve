...
// Initialisation for N-dependent resummation parameter + Mellin contour
  ...
  if(resuminfo.qT_linear == 1){
    int qT_found = 0;
    for(unsigned int ii=0; ii<resuminfo.histo_data.size(); ii++){
      std::string obs;
      std::vector<double> bin_edges;
      int t1 = read_histo_data(resuminfo.histo_data[ii], obs, bin_edges);
      if(obs.compare("qT") == 0){
        qT_found = 1;
        resuminfo.qT_vals = bin_edges;
      }
    }
    if(qT_found == 0){
      std::cout << "qT discretisation selected without providing qT binning: STOPPING";
      std::cout << std::endl;
      exit(EXIT_FAILURE);
    }
    std::cout << "qT_vals: ";
    for(unsigned int ii=0; ii<resuminfo.qT_vals.size(); ii++){
      std::cout << resuminfo.qT_vals[ii] << " ";
    }
    std::cout << std::endl;
  }
  else if(resuminfo.qT_logscale == 1){
    int qT_found = 0;
    for(unsigned int ii=0; ii<resuminfo.histo_data.size(); ii++){
      std::string obs;
      std::vector<double> bin_edges;
      int t1 = read_histo_data(resuminfo.histo_data[ii], obs, bin_edges);
      if(obs.compare("logqT") == 0){
        qT_found = 1;
        resuminfo.qT_vals = bin_edges;
      }
    }
    if(qT_found == 0){
      std::cout << "logqT discretisation selected without providing logqT binning: STOPPING";
      std::cout << std::endl;
      exit(EXIT_FAILURE);
    }
    std::cout << "logqT_vals: ";
