...
// Some basic SM constants
      ...
      // Discrete qTs for benchmarking
      std::string tag_0("histo");
      std::size_t pos = line.find(tag_0);
      std::stringstream linestream;
      if(pos!=std::string::npos){
        linestream << line.substr(pos + tag_0.length() + 1);
        res_1.histo_data.push_back(linestream.str());
      }
      set_def<int>(default_ints,"qT_linear",0);
      read_data<int>(input_status,default_ints,line,"qT_linear",res_1.qT_linear);
      set_def<int>(default_ints,"qT_logscale",0);
      read_data<int>(input_status,default_ints,line,"qT_logscale",res_1.qT_logscale);

      // Fixed q2, eta for benchmarking
      set_def<int>(default_ints,"QQ_fix",0);
      read_data<int>(input_status,default_ints,line,"QQ_fix",res_1.QQ_fix);
      set_def<int>(default_ints,"eta_fix",0);
      read_data<int>(input_status,default_ints,line,"eta_fix",res_1.eta_fix);
