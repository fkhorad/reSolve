...
// Assign q2, qt2, eta, thetaCM, phiCM with randoms from vegas
    ...
    if (drellyan_in->DYnarrowwidthapprox!=1) {
      if(drellyan_in->res_1.QQ_fix != 1){ // Maybe should be called q2 fix in this case? Dunno
  ...
  // Fix q2 for benchmarking
        q2 = std::pow(drellyan_in->res_1.QQ_Min, 2);
        randsjacob = randsjacob * 2.*drellyan_in->res_1.QQ_Min;
  // we naturally produce dsigma/dq2, so to convert to dsigma/dq then multiply by 2q
      }
      // Maybe add a check that QQ_Min must not be >= QQ_max?
    }
    ...
    if (QT_Min > QT_Max) {
      ...
      // Fixed series of qT values for benchmarking
       if(drellyan_in->res_1.qT_linear == 1){
         int choose, n_vals;
         double bin_left, bin_right, qqtt;
         n_vals = drellyan_in->res_1.qT_vals.size()-1;
         choose = floor( n_vals * x[1] );
         bin_left = drellyan_in->res_1.qT_vals.at(choose);
         bin_right = drellyan_in->res_1.qT_vals.at(choose+1);
//
         qqtt = 1/2.*(bin_left + bin_right);
         qt2 = qqtt*qqtt;
//
//         std::cout << "qT at this event: " << qqtt << std::endl;
//         std::cout << qt2 << "  " << 2.*qqtt*n_vals*(bin_right-bin_left) << std::endl;
//         std::cout << "bins'n'stuff: " << bin_left << "  " << bin_right << std::endl;
//         std::cout << n_vals << std::endl << std::endl;
         if(qqtt>=QT_Min && qqtt<=QT_Max) randsjacob = randsjacob * 2.*qqtt *n_vals*(bin_right-bin_left);
         else randsjacob = 0;
       }
       else if(drellyan_in->res_1.qT_logscale == 1){
         int choose, n_vals;
         double bin_left, bin_right, logqT;
         n_vals = drellyan_in->res_1.qT_vals.size()-1;
         choose = floor( n_vals * x[1] );
         bin_left = drellyan_in->res_1.qT_vals.at(choose);
         bin_right = drellyan_in->res_1.qT_vals.at(choose+1);
//
         logqT = 1/2.*(bin_left + bin_right);
         qt2 = std::pow(10., 2.*logqT);
         if(qt2>=QT_Min*QT_Min && qt2<=QT_Max*QT_Max) randsjacob = randsjacob * 2.*std::log(10.)*qt2 *n_vals*(bin_right-bin_left);
         else randsjacob = 0;
//
//         std::cout << "logqT at this event: " << logqT << std::endl;
//         std::cout << qt2 << "  " << 2.*qqtt*n_vals*(bin_right-bin_left) << std::endl;
//         std::cout << "bins'n'stuff: " << bin_left << "  " << bin_right << std::endl;
//         std::cout << n_vals << std::endl << std::endl;
       }
    ...
    if(drellyan_in->res_1.eta_fix != 1){
      eta = y_Min + x[3]*(y_Max-y_Min);
      randsjacob = randsjacob*(y_Max-y_Min);
    }
    else{
      // Fix eta for benchmarking
      eta = eta_Min;
      //      std::cout << "eta here: " << eta << std::endl;
      // Note no randsjacob change needed for eta as we produce dsigma/dy already
    } // Maybe add a check that eta is in the allowed range?
